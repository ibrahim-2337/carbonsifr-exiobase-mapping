...
| **Silver labeling — quality audit** | Claude (chat) | Independently re-labeled a random 100-item subset of Qwen's silver labels under the same rule book (no prior viewing of Qwen's labels — independent re-labeling protocol) | Original plan was a full two-teacher cross-check filter with Yi-1.5-34B; substituted with smaller audit under time constraints. Claude's audit labels did **not** enter training data — used only to measure agreement rate as a label-quality signal. Agreement rate documented in writeup. |
...
- Claude was used as a **quality auditor** on a 100-item sample (independent re-labeling under the rule book), and for code authoring, planning, prose drafting, and rule book extension — **never as a labeler whose outputs entered the training data or evaluation set**.
