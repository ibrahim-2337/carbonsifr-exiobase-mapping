...
| **LoRA training setup** | Claude (chat) | Drafted Unsloth + TRL `SFTTrainer` configuration | Confirmed Unsloth API matches current docs; pinned library versions; chose `r=16, alpha=16, lr=2e-4, 2 epochs` after considering tradeoffs; verified train/test split is frozen and disjoint (no leakage) |
| **Evaluation harness** | Claude (chat) | Drafted the metrics functions (exact-match + sector-match accuracy, robust JSON parsing) | Ran on the 20 CarbonSifr gold labels first as a sanity check; hand-verified the JSON parsing logic |
...
- The train/val/test split is **frozen and disjoint** — the fine-tuned adapter is evaluated only on items it never trained on, so the reported silver-51 numbers are a genuine held-out measurement.
...
- **Proprietary models were used as reviewers, disclosed for transparency.** Claude audited 100 items (its labels did **not** enter training). Gemini 2.5 Pro cross-checked 500 items; where its label was judged more accurate than Qwen's during adjudication, it was adopted — **124 of the 600 training labels (~21%) originate from Gemini**. The brief permits LLM-assisted labeling; this is noted explicitly rather than hidden. A strictly open-source-only training set is a one-line change (always keep Qwen's label on disagreement) and is listed as a variant.
- Claude was also used for code authoring, planning, prose drafting, and rule-book extension — never as a labeler whose outputs entered the training or evaluation set.
